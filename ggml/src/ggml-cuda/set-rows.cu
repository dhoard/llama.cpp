...
// ============================================================
// TurboQuant specialized set-rows kernel
// ============================================================
...
//   4. Each thread scalar-quantizes its element and packs into blocks

// Device-side codebook references for TurboQuant.
...
// Turbo3 set-rows kernel: processes 128-element chunks
...
static __global__ void k_set_rows_turbo3(
    ...
    __shared__ float smem[TURBO_HEAD_DIM];
    __shared__ float reduction[TURBO_HEAD_DIM];
    ...
    const int64_t elem_base = chunk_global * TURBO_HEAD_DIM;
    ...
    // Step 3: Scalar quantize and pack into turbo3 blocks
...
// Turbo4 set-rows kernel: processes 128-element chunks
template <typename idx_t>
static __global__ void k_set_rows_turbo4(
    ...
    // Step 3: Scalar quantize and pack into turbo4 blocks
...
static void set_rows_cuda_turbo3(
    ...
    GGML_ASSERT(ne00 % TURBO_HEAD_DIM == 0);
    const int64_t ne_total_chunks = (ne00 * ne01 * ne02 * ne03) / TURBO_HEAD_DIM;
    const dim3 grid_size((int)ne_total_chunks);
    const dim3 block_size(TURBO_HEAD_DIM);
